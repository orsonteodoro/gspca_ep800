...
static void setgain(struct gspca_dev *gspca_dev)
{
	...
	/* red color gain */
	ep800_set_feature(gspca_dev, HV7131_REG_ARCG, gain);
	/* green color gain */
	ep800_set_feature(gspca_dev, HV7131_REG_AGCG, gain);
	/* blue color gain */
	ep800_set_feature(gspca_dev, HV7131_REG_ABCG, gain);
}

static void setexposure(struct gspca_dev *gspca_dev)
{
	struct sd *sd = (struct sd *) gspca_dev;
	unsigned int integration = sd->ctrls[EXPOSURE].val << 6;
...
static int sd_start(struct gspca_dev *gspca_dev)
{
	...
	epcam_setsize(gspca_dev, gspca_dev->width, gspca_dev->height);
...
{
	struct sd *sd = (struct sd *) gspca_dev;
	int datasize=gspca_dev->width*gspca_dev->height;
...
//		return;
	...
	if (gspca_dev->height%4)
		blineoffset=1;
	bline=frame->curpix/gspca_dev->width+blineoffset;
...
{
	struct sd *sd = (struct sd *) gspca_dev;
	int linelength=gspca_dev->width;
	int i;
	...
	if (sd->eplite_curpix>=linelength) {
		...
		if (sd->eplite_curline>=gspca_dev->height*linelength)
			sd->eplite_curline=0;
	}
}
